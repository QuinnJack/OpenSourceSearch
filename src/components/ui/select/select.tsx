...
import { SelectContext, selectSizes } from "./select-context";
...
const renderIcon = (IconCandidate?: FC | ReactNode, props: Record<string, unknown> = {}) => {
    if (!IconCandidate) {
        return null;
    }

    if (isReactComponent(IconCandidate)) {
        const IconComponent = IconCandidate;
        return <IconComponent {...props} />;
    }

    if (isValidElement(IconCandidate)) {
...
const SelectValue = ({ isOpen, isFocused, isDisabled, size, placeholder, placeholderIcon, ref }: SelectValueProps) => {
    return (
        ...
        >
            ...
            >
                {(state) => {
                    const iconCandidate = state.selectedItem?.icon || placeholderIcon;
                    return (
                        <>
                            {state.selectedItem?.avatarUrl ? (
                                <Avatar size="xs" src={state.selectedItem.avatarUrl} alt={state.selectedItem.label} />
                            ) : (
                                renderIcon(iconCandidate, { "data-icon": true, "aria-hidden": true })
                            )}
